...
type ProfileCardProps = {
  ...
  /** small avatar (falls back to avatarUrl) */
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  miniAvatarUrl,
  name = "VEX-09 Specter",
  title = "Edge Division File: Wraith Protocol",
  handle = "Specter_AI",
  status = "Online",
  ...
  const animationHandlers = useMemo(() => {
    ...
    const createSmoothAnimation = (
      ...
      const loop = (t: number) => {
        ...
        const currentX = adjust(e, 0, 1, startX, targetX);
        const currentY = adjust(e, 0, 1, startY, targetY);
  ...
  const cardStyle = useMemo<React.CSSProperties>(
    () => ({
      // pass urls into CSS custom properties used by ProfileCard.css
      // (fall back to 'none' if disabled)
      ["--icon" as any]: iconUrl ? `url(${iconUrl})` : "none",
      ["--grain" as any]: grainUrl ? `url(${grainUrl})` : "none",
      ["--behind-gradient" as any]:
        showBehindGradient ? behindGradient ?? DEFAULT_BEHIND_GRADIENT : "none",
